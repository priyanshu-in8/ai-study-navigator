...
const Analytics = () => {
  ...
  const loadAnalytics = async () => {
    try {
      const data = await authApi.getAccuracyStats();

      /* ----------------------------
         Weekly Chart Data
      ----------------------------- */
      const trend =
        data.weeklyTrend?.map((item: any) => ({
          day: item.day,
          hours: Number(
            (item.accuracy / 20).toFixed(1)
          ), // fake study hours from accuracy
          score: item.accuracy,
        })) || [];
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 lg:p-8 pb-24 md:pb-8">
      ...
      <motion.div
        ...
        className="glass p-5 hover-glow max-w-lg"
      >
        <h3 className="text-sm font-semibold text-foreground mb-4">
